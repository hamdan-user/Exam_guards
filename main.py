...
# }
    ...
    response_text["date"] = datetime.now().strftime('%Y-%m-%d')
    # return json.dumps(response_text, indent=4)
...
async def generate_cv(request: Request):
    ...
    print("----data---------------------",data)
    description = data.get("cv_description")
    ...
    cv_pointers = [
      {
        "companyName":"Systems",
       "work": "computer engineer in systems. sentences that are organized and coherent,",
